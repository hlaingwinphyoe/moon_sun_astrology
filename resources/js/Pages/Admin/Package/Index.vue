<template>
  ...
  <AuthenticatedLayout>
    <div class="p-4 container mx-auto overflow-x-auto">
      <h4 class="text-lg font-bold mb-3 ml-1">Packages</h4>
...
export default {
  props: ["packages", "astrologers", "currencies"],
  components: {
    AuthenticatedLayout,
    Dialog,
    Plus,
  ...
  setup() {
    ...
    const changeStatus = (row) => {
      ElMessageBox.confirm(
        "Are you sure want to change status of this package?",
        "Confirmation",
        {
        ...
        .then(() => {
          router.patch(
            route("admin.packages.change-status", row.id),
            {},
            {
              preserveState: true,
              onSuccess: (page) => {
    ...
    const getData = () => {
      ...
      router.get("/admin/packages", state.param, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
        onFinish: () => {
